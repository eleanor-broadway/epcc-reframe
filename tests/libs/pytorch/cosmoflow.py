...
class CosmoFlowCPUtest(MLPerfBase):
    ...
    def setup_job(self):
        ...
        if part == "archer2:compute":
            ...
            self.env_vars = {"OMP_NUM_THREADS": str(self.num_cpus_per_task), "TF_ENABLE_ONEDNN_OPTS": "1"}
        train_script = os.path.join(self.mlperf_benchmarks.stagedir, "hpc", "cosmoflow", "train.py")
        cosmo_config = os.path.join(self.mlperf_benchmarks.stagedir, "hpc", "cosmoflow", "configs", "cosmo.yaml")
